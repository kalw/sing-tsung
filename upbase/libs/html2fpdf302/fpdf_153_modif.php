...
// Merged with v1.53 by Rick Garcia [rickg22@users.sourceforge.net]
...
//----- EDITEI - Put new variables here -----
var $outline_on;         // EDITEI - rick_g22 - Explicit declaration for outline mode
                         // (see SetTextOutline function)
...
function SetFont($family,$style='',$size=0)
{
    ...
    if(method_exists($this,'_preprocessfontfamily'))
        $family=$this->_preprocessfontfamily($family);
    if($family=='arial')
        $family='helvetica';
...
// EDITEI Renato A.C. - Added Superscript and Subscript Y coordinate adjustment
// Also added outline text check
function Cell($w,$h=0,$txt='',$border=0,$ln=0,$align='',$fill=0,$link='',$currentx=0) //EDITEI
{
    ...
    if($txt!=='')
    {
        ...
        elseif($align=='L' || $align=='J') // EDITEI Renato A.C.
            $dx=$this->cMargin;
        else
            $dx = 0;
        if($this->ColorFlag)
        ...
        // EDITEI Renato A.C. - Check whether we are going to outline text or not
        if($this->outline_on)
        {
    ...
    $this->lasth=$h;
    if( strpos($txt,"\n") !== false)
        $ln=1; //EDITEI - cell now recognizes \n! << comes from <BR> tag
    if($ln>0)
    {
        //Go to next line
        $this->y += $h;
        ...
        {   //EDITEI
            //Move to next line
            if ($currentx != 0) $this->x=$currentx;else
            $this->x=$this->lMargin;
        }
    }
    else
...
{
    if(!is_numeric($currentx) && $link=='')
    {
      // EDITEI RICK - Fixes incompatibility bug (blame Renato A.C.)
      $link=$currentx;
      $currentx=0;
    }
