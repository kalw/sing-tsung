...
class mcpeardb {
  ...
  function mcpeardb ($dsn = FALSE) {
	...
	// dump(get_class_methods('mcpeardb'));
	// aggregate($this, get_class($this->db)); 
  }


  /*
  function mcpeardb ($db) {
	$this->pagination = FALSE;
	$this->pagination_lock = FALSE;
	$this->last_numrows = FALSE;
	$this->verify($db);
	$this->db = $db;
    if ($this->db) {
      // Set default fetch mode to associative
      $this->db->setFetchMode(DB_FETCHMODE_ASSOC);
    } // if the db object exists
  } // function mcpeardb
  */
  ...
  function db_query ($sql){
	if($this->pagination){
		$this->pagination = FALSE;
		$countquery = 'SELECT COUNT(*) FROM ('.$sql.') derived_table';
		...
		if($res = $this->db->limitquery($sql, $lowerlimit, $this->page_size)){
  ...
  function verify (&$result) {
    global $app, $upbase_log_file_directory, $upbase_debugging;
    if (DB::isError($result)){
		trigger_error('A database query has failed',E_USER_WARNING);
		if($upbase_debugging){
			trigger_error($result->getMessage()."<pre>".$result->getUserInfo()."</pre>",E_USER_WARNING);
		}
		else{
			 ...
			 $app->log('peardb.log',$result->getUserInfo());
			 $app->log('peardb.log',$result->getMessage());
		}
	  return FALSE;
    } else {
      return $result;
  ...
  function autoCommit($onoff = false) {
    $rv = $this->db->autoCommit; 
	$this->db->autoCommit = (bool) $onoff; 
    return $rv;
  }
